...
class UserController{
    ...
    async changeImage(req,res,next){
        ...
        renderOrderPage(req, res, next){
            order.find({userID: req.user._id}, function(err, doc){
                if(doc){
                    // console.log(doc)
                    res.render('order-list', {orderList: doc})
                }
            })
        }
        ...
        cancleOrder(req, res, next){
            ...
            order.findOneAndDelete({userID: req.user._id, _id: orderId, state: 'waiting'}, function(err, doc){
                if(doc){
        ...
        recievedProduct(req, res, next){
            const orderId = req.query.orderId 
            order.findOneAndDelete({userID: req.user._id, _id: orderId, state: 'shipping'}, function(err, doc){
                if(doc){
                    res.redirect('/user/order-list')
                } else {
                    console.log(err)
